...
def main():
   ...
   # env_train = PortfolioEnv(train_set_obs, df_close_train, df_open_train)
    model = PPO(train_set_obs.shape[1], 32)
    ...
    while (n_epi != 3000):
        ...
        s = env_train.reset(True)  # observation
        ...
        while (done == False):
            ...
            weight = np.zeros(32)
            weight[action] = 1
            next_s, r, done,mk, _ = env_train.step(weight)
            pr = r/10
        ...
        if n_epi % 1 == 0:
            ...
            s = env_test.reset(False)  # observation
            done = False
            t = 0
            test_reward_list = 1
            while (done == False):
                ...
                weight = np.zeros(32)
                weight[action] = 1
                next_s, r, done, mk, _ = env_test.step(weight)
                r = r/10
                a = a + 1
    ...
    plt.plot(y, 'b')
    p1 = "./image/train_image_DAX_test(new)"
    plt.savefig(p1)
    plt.figure()
    ...
    plt.plot(y, 'b')
    p2 = "./image/test_image_DAX_test(new)"
    plt.savefig(p2)
    plt.show()
